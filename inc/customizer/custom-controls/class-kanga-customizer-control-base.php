...
if ( ! class_exists( 'Kanga_Customizer_Control_Base' ) ) {
	...
	class Kanga_Customizer_Control_Base {
		...
		public function enqueue_scripts() {
			...
			$file_rtl    = ( is_rtl() ) ? '-rtl' : '';
			$css_uri     = KANGA_THEME_URI . 'inc/customizer/custom-controls/assets/css/' . $dir_name . '/';
			$js_uri      = KANGA_THEME_URI . 'inc/customizer/custom-controls/assets/js/' . $dir_name . '/';

			wp_enqueue_style( 'custom-control-style' . $file_rtl, $css_uri . 'custom-controls' . $file_prefix . $file_rtl . '.css', null, KANGA_THEME_VERSION );
			wp_enqueue_script( 'custom-control-script', $js_uri . 'custom-controls' . $file_prefix . '.js', array( 'jquery', 'customize-base', 'kanga-color-alpha', 'jquery-ui-tabs', 'jquery-ui-sortable' ), KANGA_THEME_VERSION, true );
