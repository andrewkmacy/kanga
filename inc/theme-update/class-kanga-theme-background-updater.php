<?php
/**
 * Theme Batch Update
 *
 * @package     Kanga
 * @author      Kanga
 * @copyright   Copyright (c) 2020, Kanga
 * @link        https://wpkanga.com/
 * @since 2.1.3
 */

if ( ! class_exists( 'Kanga_Theme_Background_Updater' ) ) {

	/**
	 * Kanga_Theme_Background_Updater Class.
	 */
	class Kanga_Theme_Background_Updater {

		/**
		 * Background update class.
		 *
		 * @var object
		 */
		private static $background_updater;

		/**
		 * DB updates and callbacks that need to be run per version.
		 *
		 * @var array
		 */
		private static $db_updates = array(
			'2.1.3' => array(
				'kanga_submenu_below_header',
			),
			'2.2.0' => array(
				'kanga_page_builder_button_color_compatibility',
				'kanga_vertical_horizontal_padding_migration',
			),
			'2.3.0' => array(
				'kanga_header_button_new_options',
			),
			'2.3.3' => array(
				'kanga_elementor_default_color_typo_comp',
			),
			'2.3.4' => array(
				'kanga_breadcrumb_separator_fix',
			),
			'2.4.0' => array(
				'kanga_responsive_base_background_option',
				'kanga_update_theme_tablet_breakpoint',
			),
			'2.4.4' => array(
				'kanga_gtn_full_wide_image_group_css',
			),
		);

		/**
		 *  Constructor
		 */
		public function __construct() {

			// Theme Updates.
			if ( is_admin() ) {
				add_action( 'admin_init', array( $this, 'install_actions' ) );
			} else {
				add_action( 'wp', array( $this, 'install_actions' ) );
			}

			// Core Helpers - Batch Processing.
			require_once KANGA_THEME_DIR . 'inc/lib/batch-processing/class-wp-async-request.php';
			require_once KANGA_THEME_DIR . 'inc/lib/batch-processing/class-wp-background-process.php';
			require_once KANGA_THEME_DIR . 'inc/theme-update/class-wp-background-process-kanga-theme.php';

			self::$background_updater = new WP_Background_Process_Kanga_Theme();

		}

		/**
		 * Check Cron Status
		 *
		 * Gets the current cron status by performing a test spawn. Cached for one hour when all is well.
		 *
		 * @since 2.3.0
		 *
		 * @return true if there is a problem spawning a call to Wp-Cron system.
		 */
		public function test_cron() {

			global $wp_version;

			if ( defined( 'DISABLE_WP_CRON' ) && DISABLE_WP_CRON ) {
				return true;
			}

			if ( defined( 'ALTERNATE_WP_CRON' ) && ALTERNATE_WP_CRON ) {
				return true;
			}

			$cached_status = get_transient( 'kanga-theme-cron-test-ok' );

			if ( $cached_status ) {
				return false;
			}

			$sslverify     = version_compare( $wp_version, 4.0, '<' );
			$doing_wp_cron = sprintf( '%.22F', microtime( true ) );

			$cron_request = apply_filters(
				'cron_request',
				array(
					'url'  => site_url( 'wp-cron.php?doing_wp_cron=' . $doing_wp_cron ),
					'args' => array(
						'timeout'   => 3,
						'blocking'  => true,
						'sslverify' => apply_filters( 'https_local_ssl_verify', $sslverify ),
					),
				)
			);

			$result = wp_remote_post( $cron_request['url'], $cron_request['args'] );

			if ( wp_remote_retrieve_response_code( $result ) >= 300 ) {
				return true;
			} else {
				set_transient( 'kanga-theme-cron-test-ok', 1, 3600 );
				return false;
			}

			return $migration_fallback;
		}

		/**
		 * Install actions when a update button is clicked within the admin area.
		 *
		 * This function is hooked into admin_init to affect admin and wp to affect the frontend.
		 */
		public function install_actions() {

			do_action( 'kanga_update_initiated', self::$background_updater );

			if ( true === $this->is_new_install() ) {
				self::update_db_version();
				return;
			}

			$fallback    = $this->test_cron();
			$db_migrated = $this->check_if_data_migrated();

			$is_queue_running = kanga_get_option( 'is_theme_queue_running', false );

			$fallback = ( $db_migrated ) ? $db_migrated : $fallback;

			if ( $this->needs_db_update() && ! $is_queue_running ) {
				$this->update( $fallback );
			} else {
				if ( ! $is_queue_running ) {
					self::update_db_version();
				}
			}
		}

		/**
		 * Is this a brand new theme install?
		 *
		 * @since 2.1.3
		 * @return boolean
		 */
		public function is_new_install() {

			// Get auto saved version number.
			$saved_version = kanga_get_option( 'theme-auto-version', false );

			if ( false === $saved_version ) {
				return true;
			}

			return false;
		}

		/**
		 * Is a DB update needed?
		 *
		 * @since 2.1.3
		 * @return boolean
		 */
		private function needs_db_update() {
			$current_theme_version = kanga_get_option( 'theme-auto-version', null );
			$updates               = $this->get_db_update_callbacks();

			if ( empty( $updates ) ) {
				return false;
			}

			return ! is_null( $current_theme_version ) && version_compare( $current_theme_version, max( array_keys( $updates ) ), '<' );
		}

		/**
		 * Get list of DB update callbacks.
		 *
		 * @since 2.1.3
		 * @return array
		 */
		public function get_db_update_callbacks() {
			return self::$db_updates;
		}

		/**
		 * Check if database is migrated
		 *
		 * @since 2.3.1
		 *
		 * @return true If the database migration should not be run through CRON.
		 */
		public function check_if_data_migrated() {

			$fallback = false;

			$is_db_version_updated = $this->is_db_version_updated();
			if ( ! $is_db_version_updated ) {

				$db_migrated = get_transient( 'kanga-theme-db-migrated' );

				if ( ! $db_migrated ) {
					$db_migrated = array();
				}

				array_push( $db_migrated, $is_db_version_updated );
				set_transient( 'kanga-theme-db-migrated', $db_migrated, 3600 );

				$db_migrate_count = count( $db_migrated );
				if ( $db_migrate_count >= 5 ) {
					kanga_delete_option( 'is_theme_queue_running' );
					$fallback = true;
				}
			}
			return $fallback;
		}

		/**
		 * Checks if kanga addon version is updated in the database
		 *
		 * @since 2.3.1
		 *
		 * @return true if kanga addon version is updated.
		 */
		public function is_db_version_updated() {
			// Get auto saved version number.
			$saved_version = kanga_get_option( 'theme-auto-version', false );

			return version_compare( $saved_version, KANGA_THEME_VERSION, '=' );
		}


		/**
		 * Push all needed DB updates to the queue for processing.
		 *
		 * @param bool $fallback Fallback migration.
		 *
		 * @return void
		 */
		private function update( $fallback ) {
			$current_db_version = kanga_get_option( 'theme-auto-version' );

			error_log( 'Kanga: Batch Process Started!' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
			if ( count( $this->get_db_update_callbacks() ) > 0 ) {
				foreach ( $this->get_db_update_callbacks() as $version => $update_callbacks ) {
					if ( version_compare( $current_db_version, $version, '<' ) ) {
						foreach ( $update_callbacks as $update_callback ) {
							if ( $fallback ) {
								call_user_func( $update_callback );
							} else {
								error_log( sprintf( 'Kanga: Queuing %s - %s', $version, $update_callback ) ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
								self::$background_updater->push_to_queue( $update_callback );
							}
						}
					}
				}
				if ( $fallback ) {
					error_log( 'Kanga: Running migration without batch processing.' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
					self::update_db_version();
				} else {
					kanga_update_option( 'is_theme_queue_running', true );
					self::$background_updater->push_to_queue( 'update_db_version' );
				}
			} else {
				self::$background_updater->push_to_queue( 'update_db_version' );
			}
			self::$background_updater->save()->dispatch();
		}

		/**
		 * Update DB version to current.
		 *
		 * @param string|null $version New Kanga theme version or null.
		 */
		public static function update_db_version( $version = null ) {

			do_action( 'kanga_theme_update_before' );

			// Get auto saved version number.
			$saved_version = kanga_get_option( 'theme-auto-version', false );

			if ( false === $saved_version ) {

				$saved_version = KANGA_THEME_VERSION;

				// Update auto saved version number.
				kanga_update_option( 'theme-auto-version', KANGA_THEME_VERSION );
			}

			// If equals then return.
			if ( version_compare( $saved_version, KANGA_THEME_VERSION, '=' ) ) {
				do_action( 'kanga_theme_update_after' );
				kanga_update_option( 'is_theme_queue_running', false );
				return;
			}

			// Not have stored?
			if ( empty( $saved_version ) ) {

				// Get old version.
				$theme_version = get_option( '_kanga_auto_version', KANGA_THEME_VERSION );

				// Remove option.
				delete_option( '_kanga_auto_version' );

			} else {

				// Get latest version.
				$theme_version = KANGA_THEME_VERSION;
			}

			// Update auto saved version number.
			kanga_update_option( 'theme-auto-version', $theme_version );

			error_log( 'Kanga: db version updated successfully!' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log

			kanga_update_option( 'is_theme_queue_running', false );

			// Update variables.
			Kanga_Theme_Options::refresh();

			delete_transient( 'kanga-addon-db-migrated' );

			do_action( 'kanga_theme_update_after' );
		}
	}
}


/**
 * Kicking this off by creating a new instance
 */
new Kanga_Theme_Background_Updater();
